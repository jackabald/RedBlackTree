...
public class RedBlackTree<T extends Comparable<T>> {
	...
	/**
	 * resolves any red-black tree property violations that are introduced by
	 * inserting each new node into a red-black tree.
	 * 
	...
	protected void enforceRBTreePropertiesAfterInsert(Node<T> newNode) {
		...
		// if parent is null new node is the root
		if (parent == null) {
		...
		if (uncle != null && uncle.blackHeight == 0) {
			...
			uncle.blackHeight = 1;
			// recursive call to make sure red grandparent fulfills properties
			enforceRBTreePropertiesAfterInsert(grandparent);
		} else if (parent == grandparent.leftChild) {
			if (newNode == parent.rightChild) {
				// rotate left
				rotate(parent.rightChild, parent);
				parent = newNode;
			}
			// rotate right
			...
			if (newNode == parent.leftChild) {
				// right rotation
				rotate(parent.leftChild, parent);
	...
	private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
		if (child != null && parent != null) {
			if (child.parent != null && !child.parent.equals(parent)) {
				throw new IllegalArgumentException("child and parent don't have that relationship");
			}
			if (parent.leftChild != null && parent.leftChild.equals(child)) {
				// right rotation
				Node<T> oldLeft = parent.leftChild;
				parent.leftChild = oldLeft.rightChild; // set parents left child to the left's right child
				if (parent.parent == null) {
					root = oldLeft; // if parent's parent is null than the root is it's left child
			...
			if (parent.rightChild != null && parent.rightChild.equals(child)) {
				...
				parent.rightChild = oldRight.leftChild; // set parents right child to the right's left child
				if (parent.parent == null) {
					root = oldRight; // if parent's parent is null than the root is it's right child
				} else if (parent.parent.leftChild.equals(parent)) {
	...
	// Declaration of RBT variable to test
	...
	/**
	 * test to make sure inserting 1000 nodes will result in the size of 1000
	 */
	@Test
	public void test1() {
	...
	/**
	 * test to make sure nodes are colored correctly
	 */
	@Test
	public void test2() {
		RBT.insert(5);
		...
		// child of root is red
		assertEquals(0, RBT.root.leftChild.blackHeight);
	}

	/**
	 * test to make sure insert auto enforces RBT properties
	 */
	@Test
	public void test3() {
		RBT.insert(5);
		RBT.insert(3);
		RBT.insert(1);
		assertEquals("[ 3, 1, 5 ]", RBT.toLevelOrderString());
	}
